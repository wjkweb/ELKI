...
public class Ipeaksplus<O> extends AbstractDistanceBasedAlgorithm<O, Clustering<Model>>
		implements ClusteringAlgorithm<Clustering<Model>> {
	/**
	 * The logger for this class.
	 */
	private static final Logging LOG = Logging.getLogger(Ipeaksplus.class);
	 ...
	 *            Minpts parameter
	 */
	public Ipeaksplus(DistanceFunction<? super O> distanceFunction, int splitTimes, int maxk, int mink) {
	...
	public Clustering<Model> run(Database db, Relation<O> relation) {
		Clustering<Model> finalresult = new Clustering<>("Ipeaks Clustering", "Ipeaks-clustering");
		double silValue = 0.0;
		int finalKvalue = 0;
		EvaluateSilhouette ev = new EvaluateSilhouette(getDistanceFunction(), false);
		DistanceQuery<O> dq = db.getDistanceQuery(relation, getDistanceFunction());
		MaterializedRelation<O> re = (MaterializedRelation) relation;
		int[] densitys = getDensity(getSimMatrix(getOriginalData(relation), this.splitTimes));
		 int[] keepdensitys = new int[densitys.length];
		 for(int i = 0 ; i < densitys.length ; i++)
		 {
		 keepdensitys[i]=densitys[i];
		 }
		...
		for (int m = this.mink; m <= this.maxk; m++) {
			...
			// int[] label = new int[densitys.length];
			// for (int i = 0; i < label.length; i++) {
			// label[i] = -1;
			// }
			// for (int l = 0; l < m; l++) {
			// DBIDRef id = DBIDUtil.importInteger(peakIds[l]);
			// label[DBIDUtil.asInteger(id)] = 1;
			// clusterResult.get(1).add(DBIDUtil.deref(id));
			// }
			for (int j = 0; j < peakIds.length; j++) {
				DBIDRef id = DBIDUtil.importInteger(peakIds[j]);
				if (j < m) {
					System.out.println(id+":0");
					
					clusterResult.get(0).add(DBIDUtil.deref(id));
				} else {
					System.out.println(id+":1");
					clusterResult.get(1).add(DBIDUtil.deref(id));
				}
			}
			// for (int k = 0; k < keepdensitys.length; k++) {
			// double minDistinace = Math.sqrt(idList.length);
			// DBIDRef id = DBIDUtil.importInteger(keepdensitys[k]);
			// int idvalue = DBIDUtil.asInteger(id);
			// ArrayList ar1 = (ArrayList) this.dataset.get(idvalue -
			// idList[0]);
			// int cLabel = 0;
			// for (int n = 0; n < keepdensitys.length; n++) {
			// ArrayList ar2 = (ArrayList) this.dataset.get(keepdensitys[n] -
			// 1);
			// if(label[k]==-1&&label[n]!=-1)
			// {
			...
			// label[k]=cLabel;
			// System.out.print(id+" ");
			// clusterResult.get(cLabel).add(DBIDUtil.deref(id));
			// }
			// for (DBIDIter iditer = relation.iterDBIDs(); iditer.valid();
			// iditer.advance()) {
			...
			// int cLabel = 0;
			// for (int n = 0; n < m; n++) {
			// ArrayList ar2 = (ArrayList) this.dataset.get(peakIds[n] - 1);
			// double currentDistiance = getVectorDistiance(ar1, ar2);
			// {
			// if (currentDistiance < minDistinace) {
			// minDistinace = currentDistiance;
			...
			Clustering<Model> result = new Clustering<>("Ipeaks Clustering", "Ipeaks-clustering");
	...
	public int[] getPeaksIds(Relation<O> relation, int[] density) {
		...
		for (int i = 0; i < density.length; i++) {
			if (i == 0) {
				...
				for (int j = 0; j < i; j++) {
					currentValue = getVectorDistiance(dataset.get(density[i] - 1), dataset.get(density[j] - 1));
	...
	public int[][] getSplitsResult(Vector originalData, ArrayList<Double> randObserveroint) {
	...
	public int[][] getSimMatrix(Vector originalData, int splitTimes) {
		 ...
		 * System.out.print(retMatrix[k][l] + " "); } System.out.println(); }
		 */
		return retMatrix;
	}
	...
	public static class Parameterizer<O> extends AbstractDistanceBasedAlgorithm.Parameterizer<O> {
		 ...
		 * 0.
		 */
		public static final OptionID MINPTS_ID = new OptionID("splitTimes", "");
		...
		@Override
		protected Ipeaksplus<O> makeInstance() {
			return new Ipeaksplus<>(distanceFunction, splitTimes, maxk, mink);
		}
	}
