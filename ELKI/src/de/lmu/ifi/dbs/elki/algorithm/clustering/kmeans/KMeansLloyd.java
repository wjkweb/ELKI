...
public class KMeansLloyd<V extends NumberVector> extends AbstractKMeans<V, KMeansModel> {
	 ...
	 */
	public KMeansLloyd(NumberVectorDistanceFunction<? super V> distanceFunction, int k, int maxiter,
	...
	public Clustering<KMeansModel> run(Database database, Relation<V> relation) {
		...
		WritableIntegerDataStore assignment = DataStoreUtil.makeIntegerStorage(relation.getDBIDs(),
				DataStoreFactory.HINT_TEMP | DataStoreFactory.HINT_HOT, -1);
		double[] varsum = new double[k];

		IndefiniteProgress prog = LOG.isVerbose() ? new IndefiniteProgress("K-Means iteration", LOG) : null;
		DoubleStatistic varstat = LOG.isStatistics() ? new DoubleStatistic(this.getClass().getName() + ".variance-sum")
				: null;
		int iteration = 0;
		...
		for (int k = 0; k < means.size(); k++) {
         System.out.println(means.get(k));
		}
