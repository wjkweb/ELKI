...
public class DensityPeakClusterMerge<O> extends AbstractDistanceBasedAlgorithm<O, Clustering<Model>>
		implements ClusteringAlgorithm<Clustering<Model>> {
	private static final Logging LOG = Logging.getLogger(DensityPeakClusterMerge.class);

	protected DensityPeakClusterMerge(DistanceFunction<? super O> distanceFunction, double deltaThreshold,
			double rhoThreshold, double merageThreshold, double multiValue) {
		super(distanceFunction);
		this.deltaThreshold = deltaThreshold;
		this.rhoThreshold = rhoThreshold;
		this.merageThreshold = merageThreshold;
	...
	protected double merageThreshold;
	...
	public Clustering<Model> run(Database db, Relation<O> relation) {
		...
		calRho(dc);
		calDelta();
		Clustering<Model> finalResult = new Clustering<>("DensityPeakClusterMerge", "DensityPeakClusterMerge");
		...
		int[][] graphValues = new int[centerList.size()][centerList.size()];
		HashMap<Integer, Vector> mergePairwise = new HashMap<Integer, Vector>();
		for (int n = 0; n < allNeighbours.size(); n++) {
			for (int l = n + 1; l < allNeighbours.size(); l++) {
				Vector s1 = new Vector();
				for (int p = 0; p < allNeighbours.get(n).size(); p++) {
					s1.add(allNeighbours.get(n).get(p));
				}
				Vector s2 = new Vector();
				for (int q = 0; q < allNeighbours.get(l).size(); q++) {
					s2.add(allNeighbours.get(l).get(q));
				}
				int size1=s1.size();
				int size2=s2.size();
				s1.retainAll(s2);
				...
				if(size1+size2>0)
				{
				value=(double)size3/(size1+size2);
				}
				//if (s1.size() > this.merageThreshold&&value>0.01) {
					if (value>(double)this.merageThreshold/100) {
					System.out.println(value);
					Vector pairvalue = new Vector();
					pairvalue.add(n);
					pairvalue.add(l);
					mergePairwise.put(countlabel, pairvalue);
					countlabel++;
					graphValues[n][l] = 1;
					graphValues[l][n] = 1;
				}
		...
		Graph_DFS.color = new int[graphValues.length];
		Graph_DFS graph = new Graph_DFS();
		HashMap merge = graph.getAllPath(graphValues,centerList);
		Vector<ModifiableDBIDs> fclusterResult = new Vector<ModifiableDBIDs>();
		for (int i = 0; i < merge.size(); i++) {
			fclusterResult.add(DBIDUtil.newArray());
		}
		Iterator it = merge.keySet().iterator();
		int countc = 0;
		while (it.hasNext()) {
			int key = (int) it.next();
			Vector v = (Vector) merge.get(key);
			for (int i = 0; i < v.size(); i++) {
				int c = (int) v.get(i);
				fclusterResult.get(countc).addDBIDs(clusterResult.get(c));
			}
			countc++;
		}
		clusterResult.get(0).addDBIDs(clusterResult.get(1));
		for (int j = 0; j < fclusterResult.size(); j++) {
			resultList.add(fclusterResult.get(j));
			if(true)
			{
			ArrayModifiableIntegerDBIDs ad = (ArrayModifiableIntegerDBIDs)fclusterResult.get(j);
	...
	public static class Parameterizer<O> extends AbstractDistanceBasedAlgorithm.Parameterizer<O> {
		...
		public static final OptionID MerageThreshold = new OptionID("merageThreshold", "");
		...
		protected void makeOptions(Parameterization config) {
			...
			DoubleParameter merageThresholdp = new DoubleParameter(MerageThreshold) //
					.addConstraint(CommonConstraints.GREATER_THAN_ZERO_DOUBLE);
			if (config.grab(merageThresholdp)) {
				merageThreshold = merageThresholdp.getValue();
			}
		...
		@Override
		protected DensityPeakClusterMerge<O> makeInstance() {
			return new DensityPeakClusterMerge<>(distanceFunction, deltaThreshold, rhoThreshold, merageThreshold,
					multiValue);
		}
	}
}
